...
void vcom_Trace(uint8_t *p_data, uint16_t size)
{
  ...
  HAL_UART_Transmit(&husart2, p_data, size, 1000);
...
UTIL_ADV_TRACE_Status_t vcom_Trace_DMA(uint8_t *p_data, uint16_t size)
{
  ...
  HAL_UART_Transmit_DMA(&husart2, p_data, size);
  return UTIL_ADV_TRACE_OK;
...
UTIL_ADV_TRACE_Status_t vcom_ReceiveInit(void (*RxCb)(uint8_t *rxChar, uint16_t size, uint8_t error))
{
  ...
  HAL_UARTEx_StopModeWakeUpSourceConfig(&husart2, WakeUpSelection);

  /* Make sure that no UART transfer is on-going */
  while (__HAL_UART_GET_FLAG(&husart2, USART_ISR_BUSY) == SET);

  /* Make sure that UART is ready to receive)   */
  while (__HAL_UART_GET_FLAG(&husart2, USART_ISR_REACK) == RESET);

  /* Enable USART interrupt */
  __HAL_UART_ENABLE_IT(&husart2, UART_IT_WUF);

  /*Enable wakeup from stop mode*/
  HAL_UARTEx_EnableStopMode(&husart2);

  /*Start LPUART receive on IT*/
  HAL_UART_Receive_IT(&husart2, &charRx, 1);
